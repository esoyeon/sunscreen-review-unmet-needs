...
*   **Visualization**: `plotly` (Python), `ECharts` (Web Dashboard)
*   **Applicaton**: HTML/JS/CSS (Vanilla)

### 디렉토리 구조
```bash
sunblock-review-analysis/
├── data/
│   ├── raw/                # 1차 수집된 원본 데이터 (JSONL)
│   ├── processed/          # 노이즈 제거 및 태깅된 데이터 (Parquet)
│   └── llm/                # Gemini가 추출한 구조화 데이터
├── src/
│   ├── step3_0_baseline.py # 데이터 전처리 기본
│   ├── step3_1_tagging.py  # 룰 베이스 태깅
│   ├── step3_3_full_batch.py # Gemini API 호출 및 파싱 (핵심)
│   ├── step4_1_insight_report.py # 통계 집계 및 인사이트 도출
│   └── step5_build_dashboard.py # 웹 대시보드용 데이터 변환
├── report/
│   ├── site/               # 인터랙티브 웹 보고서 (index.html)
│   └── final/              # 경영진 보고용 PDF
└── config/
    └── tag_lexicon_v2.yaml # 분석용 어휘 사전
```
...
```bash
# 1. 룰 기반 전처리
python -m src.step3_0_baseline

# 2. LLM 추출 실행 (API 과금 주의)
python -m src.step3_3_full_batch

# 3. 리포트 생성
python -m src.step4_1_insight_report
python -m src.step5_build_dashboard
```

### 결과물 확인
웹 대시보드를 로컬에서 실행하여 결과를 확인합니다.
```bash
python -m http.server 8000
# http://localhost:8000/report/site/ 접속
```
